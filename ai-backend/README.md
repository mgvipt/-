...
> сервер (Hetzner), `ffmpeg`, ключ **Claude API** и ключ для транскрибации (**OpenAI Whisper**).
> Без ключей и сервера запустить нельзя — это и есть backend-этап.

## Что делает

`POST /process` (multipart: `video` — файл, `roomName` — необязательно):
1. `ffmpeg` извлекает аудио из видео.
2. Whisper транскрибирует речь **с таймкодами**.
...
- Доступ к серверу Hetzner (SSH) — куда деплоить.
- Ключ **Claude API** (`ANTHROPIC_API_KEY`).
- Ключ транскрибации (**OpenAI** для Whisper) — или поднимем self-hosted whisper.cpp без OpenAI.
