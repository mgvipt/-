...
REVIEW_TASK_BASE = """Ти — команда перевірки рилса перед публікацією. Три ролі дивляться ролик кожна зі свого боку:
...
from .content_rules import HEADLINE_RULE as _HEADLINE, VOICE_RULES as _VOICE  # noqa: E402 — єдині правила мови й заголовків (27.09)
REVIEW_TASK = REVIEW_TASK_BASE + "\n\nПеревіряй також: чи є заголовок одного з типів і чи мова розмовна.\n" + _HEADLINE + "\n" + _VOICE
...
- паузу — комою або «...»;
- інтонацію кінця фрази чи слова — розділовим знаком: «?» — голос угору, «.» — спокійно донизу, «!» — наголошено;
...
def fix_narration(reel, instruction):
    """Виправити вимову в суцільній озвучці (27.09): Groq переписує текст для голосу (brief.narration_tts), екран не змінюється."""
    from . import freeai
    brief = dict(reel.brief or {})
    base = (brief.get("narration_tts") or brief.get("narration") or "").strip()
    instruction = (instruction or "").strip()
    if not base or len(instruction) < 3:
        raise ValueError("Напишіть, що саме звучить неправильно.")
