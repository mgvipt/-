...
def library_product(request, item):
    ...
    if not hasattr(request, "_library_products"):
        ...
        request._library_products = {
            p.id: p for p in Product.objects.filter(pk__in=ids).only("id", "name", "sku", "price", "currency", "is_active", "shop_specs", "description", "updated_at")
        }
...
def cezar_message(request, items):
    ...
    if len(ids) != 1 or None in ids or not items:
        raise ValueError("Оберіть фото одного товару Cezar")
    info = library_product(request, items[0])
    if not info or not info.get("available") or not info.get("length"):
