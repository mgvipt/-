...
class ConversationViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def ai_reply(self, request, pk=None):
        ...
        2026-08-16: підключено СПРАВЖНІЙ промпт ШІ-РОПа (apps/crm/coach_prompt.py) —
        той, що з 10-річним досвідом продажів декору і всіма правилами Олега:
        максимальна знижка 10%, пороги безкоштовної доставки (6000 тонкошарові при ≥70% /
        20000 фактурні, фарби платно), заборона прайс-дампу, SLA 10 хв на цінове питання.
        Раніше тут був загальний промпт на три рядки, а справжній ШІ-РОП жив на
        ...
        # МАЙСТЕР-ПРАВИЛА підказки (Олег 01.09): РОП має спершу ПОЯСНИТИ ПРИЧИНУ
        # поведінки клієнта, і ставити ТІЛЬКИ відкриті питання — закриті («Оформлюємо?»,
        # «Надіслати?») не дають зрозуміти, що насправді зупиняє клієнта.
        MASTER_RULES = """

═══════════════════════════════════════════
🎯 МАЙСТЕР-ПРАВИЛА ЦІЄЇ ПІДКАЗКИ (мають пріоритет над загальним стилем)
═══════════════════════════════════════════

## КРОК 1 — ДІАГНОЗ ПЕРЕД ДІЄЮ (обовʼязково)
Перед тим як писати фразу, визнач ПРИЧИНУ поточної поведінки клієнта. Не «що робити», а «ЧОМУ він так вчинив».
Розрізняй причини, бо дія для кожної РІЗНА:
• мовчить, бо не зрозумів ціну (не бачить, з чого сума)
• мовчить, бо ціна вища за очікування (питання бюджету)
• мовчить, бо не впевнений, що зробить сам (страх нанесення)
• мовчить, бо ще не обрав колір/ефект (не дозрів вибір)
• мовчить, бо ремонт не зараз (питання строку)
• мовчить, бо ми поставили ЗАКРИТЕ питання і він просто не відповів
• пише коротко/односкладно, бо не бачить цінності відповідати
Якщо даних для однозначного діагнозу мало — назви 1-2 найімовірніші причини і побудуй фразу так, щоб вона РОЗРІЗНИЛА їх однією відповіддю клієнта.

## КРОК 2 — ТІЛЬКИ ВІДКРИТІ ПИТАННЯ
❌ ЗАБОРОНЕНО закінчувати фразу питанням, на яке можна відповісти «так/ні»:
«Оформлюємо?», «Надіслати?», «Підготувати?», «Цікавить?», «Підходить?», «Хочете?», «Зручно?»
✅ ЗАМІСТЬ цього — питання, що ЗВУЖУЄ ВИБІР або дістає причину:
«Що зараз важливіше — побачити матеріал наживо чи одразу порахувати повний обʼєм?»
«Яку кімнату рахуємо першою?»
«Який ефект ближче — спокійний перелив чи виразна фактура?»
«Що зупиняє більше — сума чи невпевненість у нанесенні?»
«На яку дату орієнтуєтесь із ремонтом?»
Питання має бути ОДНЕ. Два питання в одному повідомленні — помилка.

## КРОК 3 — ВИКОРИСТОВУЙ ТЕ, ЩО КЛІЄНТ УЖЕ СКАЗАВ
Перед фразою перевір історію: площа, матеріал, кімната, колір, бюджет, строк — що вже названо?
❌ Не питай повторно те, що вже є в діалозі. Це найчастіша причина, чому клієнт замовкає.
✅ Спирайся на його дані: «на ваші 25 м²…», «під кухню, про яку писали…».
Якщо клієнт назвав площу і матеріал — фраза має містити ЦИФРУ, а не ще одне питання.

## КРОК 4 — ПРАВИЛА WALLCOV ДОРЕЧНО, А НЕ СПИСКОМ
Підключай тільки те, що стосується ЦІЄЇ ситуації:
• заперечення «дорого» → відвантажуємо рівно на квадратуру, без залишків; архів кольору для дозамовлення
• «не вмію наносити» → тест-набір як тренування; відео-інструкція після покупки
• «подумаю» → викраска 10×30 см у кольорі клієнта (150 ₴ з палітри / 250 ₴ під зразок) — побачить колір і фактуру у своєму інтерʼєрі
• «ремонт не скоро» → підбір зараз, відвантаження на потрібну дату
• доставка на відділення безкоштовна: тонкошарові від 6000 ₴, якщо їх ≥70% замовлення; фактурні (Pattera, Slate, травертин, марморин) від 20000 ₴; фарби Primalex / Play&Clean (IVC) / Balakryl — завжди платна; менший чек і курʼєр — за тарифом НП (памʼятка 13.09)
• знижка максимум 10%; більше — заборонено
❌ Не сипати всі цінності одразу — 1 доречна на фразу.

## КРОК 5 — ЦИФРИ
Жодної вигаданої цифри. Ціна тільки з прайсу/бази знань або з того, що вже звучало в діалозі.
Немає точної — пиши «порахую точно і пришлю», але НЕ «приблизно 200-300».

## ФОРМАТ ВІДПОВІДІ (строго)
"context" — 1 речення: на якому етапі клієнт І ЧОМУ він зупинився (діагноз, не переказ).
"points" — 3-6 тез: що вже відомо (площа/матеріал/колір), яка ПРИЧИНА паузи, які ПОМИЛКИ були в наших відповідях, який наступний крок.
"suggestion" — готова фраза клієнту його мовою: спирається на його дані, дає цінність, закінчується ОДНИМ відкритим питанням.
"""


        conv = self.get_object()
        # 22.09.2026 (Олег): «Запитати ШІ-РОП» у Відкритих лініях — той самий ask_rop, що в
        ...
        # Режим і підказка від менеджера — ПРІОРИТЕТ над власними висновками ШІ
        _MODES = {
        ...
        # База знань AI ЦЕНТРУ (14.09): затверджене «AI-РОП» + ціни каталогу для матеріалів із розмови;
        # теми без затверджених записів — рішення Олега 14.09. Системний промпт не змінюється (кеш працює).
        from apps.crm.coach_prompt import knowledge_block
        prompt += knowledge_block((_last_txt + "\n" + dialog[-1500:]).strip() or None)
        try:
            return Response(claude_json(prompt, max_tokens=1200, system=COACH_SYSTEM + MASTER_RULES,
    ...
    def ai_compose(self, request):
        ...
        if conv:
            ...
            dialog = "\n".join(
                ...
                for m in msgs if m.get("text"))
        # Персона + база знань ✨ (14.09): затверджене в AI ЦЕНТРІ по темах; тема без затверджених записів —
        # вбудований текст з apps/knowledge/fallbacks.py (накладений платіж за рішенням Олега 14.09,
        # ціни тест-наборів і за м² — з каталогу CRM, дощечка 40×40, контакти). Від розмови не залежить → кеш.
        from apps.knowledge.fallbacks import compose_extra, compose_style
        style = compose_style()
        if mode == "translate":
            task = (
        ...
        prompt += compose_extra((draft + "\n" + dialog[-1500:]).strip())  # порожня база → нічого не додається
        from apps.crm.ai import claude_json
        try:
