...
PERMISSION_GROUPS = [
    ...
    ("Фінанси · дії", [
        ...
        ("finance.day.close", "Закривати день (знімок платежів)", "Кнопка «Закрити день» у Фінанси → Знімки дня. Автознімок о 18:00 працює без цього права"),
        ("finance.day.edit_closed", "Змінювати суму/дату/рахунок у ЗАКРИТИХ днях", "Бухгалтер/власник: перезнімок і «відкрити день». Без права після закриття дня змінюються лише категорія, коментар, контрагент, угода"),
    ...
    ("Склад", [
        ...
        ("warehouse.work", "Меню «Відвантаження» (робоче місце складу)", "Черга відвантажень, мої задачі, розлив, ЗП складу. Менеджерам продажів зазвичай не потрібне — лише «Складський облік»."),
    ...
    ("Платежі і документи", [
        ...
        ("payment.method.cash", "Спосіб оплати: Готівка", "Якщо в ролі НЕ позначено ЖОДНОГО «Спосіб оплати» — у вікні «Прийняти оплату» доступні всі способи (як раніше). Позначте потрібні — решта зникне"),
        ("payment.method.liqpay", "Спосіб оплати: LiqPay (посилання на оплату картою онлайн)", ""),
        ("payment.method.requisites", "Спосіб оплати: Реквізити IBAN", ""),
        ("payment.method.np", "Спосіб оплати: Накладений платіж", ""),
        ("payment.method.installment", "Спосіб оплати: Розстрочка Приват", ""),
        ("payment.method.terminal", "Спосіб оплати: Термінал (картка/NFC)", ""),
        ("payment.method.credit", "Спосіб оплати: Товарний кредит (салон)", ""),
        ("payment.method.advance", "Спосіб оплати: З авансу клієнта", ""),
        ("payment.method.journal", "Спосіб оплати: З журналу (гроші вже прийшли)",
         "Привʼязати до сделки прихід, який уже є у фінансовому журналі (банк, каса) — без нової операції і без чека. "
         "Лише за явним правом: правило «жодного способу не позначено → усі» на нього НЕ діє"),
    ]),
...
# ── Ставки співробітників (додано 2026-09-14) ──
PERMISSION_GROUPS.append((
    "Ставки співробітників",
    [
        ("payroll.rates.view", "Бачити ставки співробітників, ЗП за ставками і точку беззбитковості зі ставками",
         "Налаштування → Ставки співробітників. За замовчуванням — лише власник"),
        ("payroll.rates.edit", "Змінювати ставки (нова версія з місяця), вносити акти",
         "Минулі місяці не перераховуються — зміна діє з обраного місяця"),
        ("objects.act.close", "Закривати акти обʼєктів",
         "Закритий акт нараховує % менеджеру обʼєкта. Рішення Олега 14.09 — лише власник"),
    ],
))

# ── База знань ШІ-агентів, AI ЦЕНТР (додано 2026-09-14) ──
PERMISSION_GROUPS.append((
    "База знань ШІ",
    [
        ("knowledge.view", "Бачити базу знань ШІ", "Переглядати записи бази знань і що бачить кожен ШІ-агент"),
...
# ── Маржа і економіка угоди (додано 2026-09-14, margin-perms) ──
PERMISSION_GROUPS.append((
    "Маржа угоди",
    [
        ("deal.margin.view", "Бачити маржу угоди (суму і %)",
         "Без права у картці угоди видно лише «Ваш заробіток з угоди» — без суми маржі й %. "
         "За замовчуванням — лише власник"),
        ("deal.economics.view", "Бачити блок «Економіка угоди» (собівартість, доставка, комісія…)",
         "Повний розклад угоди: виручка − собівартість − доставка − комісія − пакування − майстри = маржа. "
         "Хто бачить економіку, бачить і маржу. За замовчуванням — лише власник"),
    ],
))
...
# ── Партнерська програма (додано 2026-09-14) ──
PERMISSION_GROUPS.append((
    "Партнерська програма",
    [
        ("partners.view", "Розділ «Партнери»", "Список партнерів і кандидатів, рівні та знижки (перегляд)"),
        ("partners.manage", "Налаштовувати рівні та партнерські знижки",
         "Рівні, пороги, знижки на папки й товари, налаштування. За замовчуванням — лише власник"),
        ("partners.assign", "Присвоювати статус партнера клієнту",
         "Галочка «Партнер» у картці клієнта і ручне ПІДВИЩЕННЯ рівня. Менеджерам НЕ давати — статус дає знижки"),
        ("partners.below_min", "Партнерська знижка нижче мінімальної маржі",
         "Виняток на товар, після якого лишається менше мінімальної маржі. Лише з причиною"),
    ],
))
