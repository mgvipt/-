...
PERMISSION_GROUPS = [
    ...
    ("Зовнішні інструменти", [
        ("zamer.access", "Застосунок «Wallcov Замер»", "Доступ до мобільного застосунку замера стін (iPhone LiDAR). Без права співробітник не може відправляти замер у CRM."),
        ("calc.access", "Калькулятор кошторисів", "Доступ до калькулятора calc.wallcovdec.com.ua (обмеження входу діятиме після звʼязку акаунтів калькулятора з CRM — Фаза 3)."),
    ]),
]
