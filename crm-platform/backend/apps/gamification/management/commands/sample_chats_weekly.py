"""Тижнева ВИПАДКОВА вибірка чатів для ІІ-розбору (Розвиток v2, 16.09.2026). ВИМКНЕНО за замовчуванням.

Навіщо: «Якість дзвінків» зараз рахується лише з дзвінків; чати ІІ сам не розбирає (analyst_auto вимкнено), а
...
class Command(BaseCommand):
    help = "Випадкова тижнева вибірка чатів для ІІ-розбору (лише якщо увімкнено власником). DRY за замовчуванням."
    ...
    def handle(self, *a, **o):
        ...
        if not s.chat_sampling:
            w("Вибірку чатів ВИМКНЕНО («Розвиток» → налаштування власника). Нічого не роблю — ІІ не викликається.")
            return
        until = timezone.now()
        ...
        if not o["live"]:
            w("DRY: ІІ не викликався. Додайте --live, щоб розібрати.")
            return
