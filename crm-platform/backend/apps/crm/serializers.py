...
class ContactSerializer(serializers.ModelSerializer):
    ...
    purchased_materials = serializers.SerializerMethodField()
    ...
    class Meta:
        ...
        fields = ["id", "first_name", "last_name", "middle_name", "nickname", "display_name", "phone",
                  ...
                  "purchased_materials", "advance_adjust", "advance_adjust_note", "expense_adjust", "expense_adjust_note"]

    def get_display_name(self, obj):
        return str(obj)

    def get_purchased_materials(self, obj):
        """Унікальні товари лише з оплачених або успішно закритих угод."""
        materials = {}
        deals = getattr(obj, "_purchased_deals", None)
        if deals is None:
            from django.db.models import Q
            deals = (obj.deals.filter(Q(stage__is_won=True) | Q(payments__is_paid=True))
                     .distinct().prefetch_related("items__product"))
        for deal in deals:
            items = getattr(deal, "_purchased_items", deal.items.all())
            for item in items:
                if item.product_id and item.product:
                    materials[item.product_id] = item.product.name
        return [{"id": product_id, "name": materials[product_id]}
                for product_id in sorted(materials, key=lambda key: materials[key].lower())]
