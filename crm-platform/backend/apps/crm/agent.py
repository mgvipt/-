...
def _move_stage(entity, kind, to_name, reason, user, autonomous):
    ...
    if kind == "deal":
        ...
        # «Домовились про оплату» ставить ТІЛЬКИ відправка реквізитів/посилання на оплату
        # через «Прийняти оплату» (send_pay_link), НЕ агент (Олег, 29.08). Агент максимум —
        # «Розрахунок здійснено (КП)». Раніше агент помилково рухав сюди, щойно менеджер
        # підготував прорахунок/документ (без жодної відправки оплати).
        if "домовил" in _tn:
            return {"ok": False, "msg": "стадію «%s» НЕ ставить агент — вона виставляється лише при відправці реквізитів/посилання на оплату через «Прийняти оплату»." % target.name}
