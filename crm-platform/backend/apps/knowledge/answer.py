...
from .seller_prompt import system_for as _seller_system_for
...
PAY_RX = re.compile(
    ...
    re.I)
CALL_RX = re.compile(r"передзвон|перезвон|зателефонуйте|позвоните|наберіть|наберите", re.I)
_NUM = r"\d+(?:[   ]\d{3})*(?:[.,]\d+)?"
...
def guard(reply, allowed, used, client_last):
    ...
    problems = []
    # 18.09.2026: раніше будь-яке «хочу замовити» відправляло діалог менеджеру. Тепер продавець
    # оформлює тест-набір сам (order → сделка + LiqPay), реквізити надсилає CRM, дзвінок і великі
    # замовлення лишились за менеджером — тому тут лишаємо тільки дзвінок.
