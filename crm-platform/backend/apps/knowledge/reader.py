"""ОДИН читач бази знань для всіх ШІ-агентів CRM.
...
def select(agent, query=None, limit=20, items=None):
    """22.09.2026 (критичний фікс, Олег: ШІ-РОП сказав «інформації про адресу немає», хоча вона
    ...
    for topic, chunk in chunks:
        if topic and topic in covered:
            if topic not in done:
                ...
                # 22.09.2026 (критичний фікс, Олег: ШІ-РОП не знав адресу компанії, хоча вона
