...
Итог: CRM откроется по адресу вроде **https://crm.gmideas.com.ua** с автоматическим
...
и доступ к **DNS** домена `gmideas.com.ua`.

---

## Шаг 1. DNS — направить поддомен на сервер
В панели, где управляется домен `gmideas.com.ua`, добавь запись:

| Тип | Имя (host) | Значение |
|-----|------------|----------|
| `A` | `crm`      | IP-адрес твоего сервера Hetzner |

Через несколько минут `crm.gmideas.com.ua` будет указывать на сервер.
...
```
Caddy сам получит HTTPS-сертификат. Открой **https://crm.gmideas.com.ua**.
