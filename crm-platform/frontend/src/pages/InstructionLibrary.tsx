...
export function InstructionLibrary({conversationId,onInsertText,onClose,onBack}:{conversationId:number;onInsertText?:(text:string)=>void;onClose:()=>void;onBack:()=>void}) {
 ...
 useEffect(()=>{api.get<{items:Instruction[];training?:Entry[]}>("/api/content-library/instructions/").then(r=>{setItems(r.items);setTraining(r.training||[]);}).catch(()=>setError("Не вдалося завантажити інструкції")).finally(()=>setLoading(false));},[]);
