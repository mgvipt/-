            ...
            <div className="panel">
              ...
              {deal.items.length === 0 ? <div className="muted" style={{ fontSize: 13 }}>{t("Товаров пока нет.","Товарів поки немає.")}</div> : (
                ...
                <thead><tr style={{ color: "#64748b", fontSize: 9.5, lineHeight: 1.1, textAlign: "left", verticalAlign: "middle", whiteSpace: "nowrap" }}>
                  <th style={{ padding: "6px 4px" }}>№</th><th style={{ padding: "6px 4px", width: "100%" }}>{t("Товар","Товар")}</th>
                  ...
                  <tbody>{(() => {
                    ...
                    <tr style={{ borderTop: "1px solid #f1f5f9" }}>
                      ...
                      <td style={{ padding: "6px 4px", position: "relative" }}>{!Number((it as any).consumption) && rooms.length > 0 ? <span title={t("У товара не заполнен «Расход на 1 м²» — впишите его в карточке товара, тогда количество посчитается автоматически","У товару не заповнено «Витрата на 1 м²» — впишіть її в картці товару, тоді кількість порахується автоматично")} style={{ position: "absolute", right: 2, top: 2, color: "#f59e0b", cursor: "help", zIndex: 2 }}><Icon n="warn" size={11} /></span> : null}
                        {editProdItem === it.id ? (
                          <>
        ...
        return (
          <div onClick={() => !refundBusy && setRefundOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 60 }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: 12, padding: 22, width: 380 }}>
              ...
              {pl?.checkbox_receipt_id && (
                <label style={{ display: "flex", alignItems: "flex-start", gap: 8, fontSize: 12.5, marginBottom: 8, cursor: "pointer" }}>
                  <input type="checkbox" checked={refundReceipt} onChange={(e) => setRefundReceipt(e.target.checked)} style={{ marginTop: 2 }} />
              ...
              {a >= maxA - 0.001 && (
                <label style={{ display: "flex", alignItems: "flex-start", gap: 8, fontSize: 12.5, marginBottom: 8, cursor: "pointer" }}>
                  ...
                  <span>↩ {t("Перевести сделку в «Возврат» и вернуть товар на склад","Перевести сделку у «Возврат» і повернути товар на склад")}</span>
                </label>
              )}
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button className="btn" style={{ flex: 2, height: 38, background: "#c2410c", color: "#fff", border: "none", fontWeight: 700 }}
                  ...
                  onClick={async () => {
                    ...
                    try {
                      const r: any = await api.post(`/api/deals/${id}/liqpay-refund/`, {
                        amount: a,
                      ...
                      flash(t(`✓ LiqPay вернул клиенту ${fmt(r.refunded)} ₴`, `✓ LiqPay повернув клієнту ${fmt(r.refunded)} ₴`)
                        + (r?.receipt?.url ? (" · " + t("чек","чек") + ": " + r.receipt.url) : ""));
                    } catch (e: any) {
