                ...
                <thead><tr style={{ color: "#64748b", fontSize: 9.5, lineHeight: 1.1, textAlign: "left", verticalAlign: "middle", whiteSpace: "nowrap" }}>
                  ...
                  <tbody>{(() => {
                    ...
                    <tr style={{ borderTop: "1px solid #f1f5f9" }}>
                      ...
                      <td style={{ padding: "6px 4px", whiteSpace: "nowrap" }}><span style={{ display: "inline-block", width: 14, textAlign: "center", verticalAlign: "middle", flex: "none" }}>{(it as any).room_name && !Number((it as any).consumption) ? <span title={t("У товара не заполнен «Расход на 1 м²» — впишите его в карточке товара, тогда количество посчитается автоматически","У товару не заповнено «Витрата на 1 м²» — впишіть її в картці товару, тоді кількість порахується автоматично")} style={{ color: "#f59e0b", cursor: "help" }}><Icon n="warn" size={12} /></span> : null}</span><input key={`q-${it.id}-${it.quantity}`} defaultValue={Number(it.quantity)} type="number" step="0.01" min={0} onBlur={(e) => Number(e.target.value) !== Number(it.quantity) && updateItem(it.id, { quantity: e.target.value })} style={{ ...editInp, width: 50 }} /></td>
