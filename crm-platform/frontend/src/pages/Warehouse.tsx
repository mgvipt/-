...
interface SheetRow { id: number; name: string; sku?: string; unit: string; opening: number; opening_recount?: number; received: number; sold: number; recount?: number; controls?: { date: string; fact: number; delta: number }[]; calc?: number; book: number; }
      ...
      {view === "inv" && (
        <div style={{ background: "#fff", borderRadius: 8, border: "1px solid #e2e8f0", padding: 22, maxHeight: "calc(100vh - 170px)", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 10, flexWrap: "wrap" }}>
              ...
              <span className="muted" style={{ fontSize: 12 }}>{t("Начальный (факт переучёта) + Поступление − Продано = Конечный расчётный. Плюс контрольные пересчёты = В системе. Расхождение = Факт − В системе.","Початковий (факт переобліку) + Надходження − Продано = Кінцевий розрахунок. Плюс контрольні переобліки = В системі. Розбіжність = Факт − В системі.")}</span>
            </div>
            ...
            <div style={{ overflowY: "auto", flex: 1 }}>
              <table style={{ width: "100%", fontSize: 13 }}>
                <thead><tr>{[t("Товар","Товар"), t("Ед.","Од."), t("Начальный","Початковий"), t("Поступ.","Надходж."), t("Продано","Продано"), t("Конечный (расчёт)","Кінцевий (розрахунок)"), t("Контрольные пересчёты","Контрольні переобліки"), t("В системе","В системі"), t("Факт","Факт"), t("Расхождение","Розбіжність")].map((h) => <th key={h} style={{ position: "sticky", top: 0, background: "#fff", zIndex: 2, boxShadow: "inset 0 -1px 0 #e2e8f0", textAlign: "left" }}>{h}</th>)}</tr></thead>
                <tbody>
                  {sheet.map((r) => {
                    ...
                    return (
                      <tr key={r.id}>
                        ...
                        <td>{r.opening.toLocaleString("ru")}{r.opening_recount ? <div className="muted" style={{ fontSize: 10 }}>{t("факт переучёта","факт переобліку")} {pFrom.split("-").reverse().slice(0,2).join(".")}</div> : null}</td>
                        ...
                        <td title={t("Контрольные пересчёты ВНУТРИ периода: дата → сколько насчитали, и сколько система дописала","Контрольні переобліки ВСЕРЕДИНІ періоду: дата → скільки нарахували, і скільки система дописала")}>{(r.controls && r.controls.length) ? r.controls.map((c) => (<div key={c.date} style={{ whiteSpace: "nowrap", fontSize: 11 }}>{c.date.split("-").reverse().slice(0,2).join(".")} → <b>{c.fact.toLocaleString("ru")}</b> <span style={{ color: c.delta > 0 ? "#16a34a" : "#dc2626" }}>({c.delta > 0 ? "+" : ""}{c.delta.toLocaleString("ru")})</span></div>)) : <span className="muted">—</span>}</td>
