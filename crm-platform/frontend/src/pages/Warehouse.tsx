...
interface Product {
  ...
  price: string; cost: string; cost_pct?: string; min_price?: string; consumption_per_m2?: string; density_kg_l?: string; currency: string; margin: number;
...
` + label + "?")) return;
  ...
  async function saveCard() {
    ...
    try {
      ...
      const upd: any = await api.patch(`/api/products/${card.id}/`, {
        ...
        price: Number(cardEdit.price) || 0, cost: Number(cardEdit.cost) || 0, cost_pct: Number(cardEdit.cost_pct) || 0, min_price: Number(cardEdit.min_price) || 0, consumption_per_m2: (String(cardEdit.consumption_per_m2 ?? "").trim() === "" ? null : Number(cardEdit.consumption_per_m2)), density_kg_l: (String(cardEdit.density_kg_l ?? "").trim() === "" ? null : Number(cardEdit.density_kg_l)), unit: cardEdit.unit,
        sku: cardEdit.sku, is_active: cardEdit.is_active, track_stock: cardEdit.track_stock, is_drop: cardEdit.is_drop,
        min_stock: Number(String(cardEdit.min_stock ?? 0).replace(",", ".")) || 0, reorder_qty: Number(String(cardEdit.reorder_qty ?? 0).replace(",", ".")) || 0, supplier: cardEdit.supplier || null,
  ...
  const [invImportRes, setInvImportRes] = useState<any>(null); // результат превью (matched/not_found)
  const [invImportImages, setInvImportImages] = useState<string[]>([]); // ужатые фото (data URL)
  const [invImportRows, setInvImportRows] = useState<any[]>([]); // РЕДАКТИРУЕМЫЕ строки превью {id,sku,name,unit,book,fact}
  const [invImportPage, setInvImportPage] = useState(1);
  ...
  // шаг 1: собрать расхождения ПО ВСЕМ страницам (общий черновик на сервере), не только по видимой
  async function conductInventory() {
    let items: any[] = [];
    try {
      const d: any = await api.get(`/api/warehouse/inventory-draft/?rows=1&to=${encodeURIComponent(pTo)}`);
      items = (d.rows || [])
        .filter((r: any) => Math.abs(Number(r.delta)) > 0.0001)
        .map((r: any) => ({ product: r.product, name: r.name, unit: r.unit, book: Number(r.book), fact: Number(r.fact), delta: Number(r.delta), quantity: Number(r.fact), price: 0 }));
    } catch {
      items = sheet
        .filter((r) => facts[r.id] !== undefined && Number(facts[r.id]) !== Number(r.book))
        .map((r) => { const q = Number(facts[r.id]); return { product: r.id, name: r.name, unit: r.unit, book: Number(r.book), fact: q, delta: q - Number(r.book), quantity: q, price: 0 }; });
    }
    if (!items.length) {
  ...
  async function printInventory() {
    ...
    try {
      ...
      const body = rows.map((r, i) => `<tr><td class="n">${i + 1}</td><td class="sku">${esc(r.sku || "")}</td><td>${esc(r.name)}</td><td class="c">${esc(r.unit)}</td><td class="r">${r.book.toLocaleString("ru")}</td><td class="fact"></td><td class="note"></td></tr>`).join("");
...
<style>
  *{box-sizing:border-box} body{font-family:-apple-system,Segoe UI,Arial,sans-serif;color:#111;margin:18px}
  ...
  th{background:#eee} td.n,th.n{width:34px;text-align:center} td.c,th.c{text-align:center;width:52px} td.sku,th.sku{width:96px;font-size:11px;color:#333}
  ...
  <table><thead><tr><th class="n">№</th><th class="sku">${t("Артикул","Артикул")}</th><th>${t("Товар","Товар")}</th><th class="c">${t("Ед.","Од.")}</th><th class="r">${t("Учёт","Облік")}</th><th class="fact">${t("Факт (вписать)","Факт (вписати)")}</th><th class="note">${t("Примечание","Примітка")}</th></tr></thead><tbody>${body}</tbody></table>
  ...
  // ── импорт инвентаризации: файл / вставка / ФОТО (ИИ) → редактируемое превью ──
  function openInvImport() { setInvImportOpen(true); setInvImportText(""); setInvImportXlsx(""); setInvImportFileName(""); setInvImportRes(null); setInvImportImages([]); setInvImportRows([]); }
  // ужать фото в JPEG (макс. 1550px, q0.8) — чтобы пролезло и лучше распозналось
  function downscaleImage(file: File): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      rd.onload = () => {
        const img = new Image();
        img.onload = () => {
          const MAX = 1550; let { width: w, height: h } = img;
          if (w > MAX || h > MAX) { const k = Math.min(MAX / w, MAX / h); w = Math.round(w * k); h = Math.round(h * k); }
          const cv = document.createElement("canvas"); cv.width = w; cv.height = h;
          const cx = cv.getContext("2d"); if (!cx) { reject(new Error("no ctx")); return; }
  ...
  async function addInvImportPhotos(files?: FileList | null) {
    if (!files || !files.length) return;
    setInvImportRes(null);
    const arr: string[] = [];
    for (const f of Array.from(files).slice(0, 20)) { try { arr.push(await downscaleImage(f)); } catch { /* skip */ } }
    setInvImportImages((prev) => [...prev, ...arr].slice(0, 20));
    setInvImportText(""); setInvImportXlsx(""); setInvImportFileName("");
  }
  function onInvImportFile(f?: File) {
    if (!f) return;
    setInvImportFileName(f.name); setInvImportRes(null); setInvImportImages([]);
  ...
  // распознать/проверить источник → заполнить редактируемое превью
  async function invImportPreview() {
    setInvImportBusy(true);
    const fromPhotos = invImportImages.length > 0;
    try {
      const body: any = { commit: false };
      if (fromPhotos) body.images = invImportImages;
      else if (invImportXlsx) body.xlsx_b64 = invImportXlsx;
      else body.data = invImportText;
      const r: any = await api.post("/api/stock-documents/import-inventory/", body);
      const fresh = (r.preview || []).map((x: any) => ({ ...x }));
      if (fromPhotos) {
        // фото: НАКАПЛИВАЕМ (можно грузить пачками), дедуп по товару — новый факт выигрывает; фото очищаем под след. пачку
        setInvImportRows((prev) => {
          const byId: Record<number, any> = {};
          for (const row of prev) byId[row.id] = row;
          for (const row of fresh) byId[row.id] = row;
          return Object.values(byId);
        });
        setInvImportImages([]);
        setInvImportRes({ matched: r.matched, not_found: r.not_found, changed: r.changed });
      } else {
        setInvImportRows(fresh);
        setInvImportRes(r);
      }
      setInvImportPage(1);
    } catch (e: any) {
      const msg = e?.response?.data?.detail || e?.message || "";
      setInvImportRes({ error: (t("Не удалось обработать. ","Не вдалося обробити. ") + msg) });
    }
    setInvImportBusy(false);
  }
  // провести по отредактированным строкам
  async function invImportCommit() {
    const items = invImportRows
      .filter((r) => r.fact !== "" && r.fact !== null && r.fact !== undefined && Number(r.fact) !== Number(r.book))
      .map((r) => ({ product: r.id, fact: Number(r.fact) }));
    if (!items.length) { setInvImportRes((p: any) => ({ ...(p || {}), error: t("Нет расхождений для проведения.","Немає розбіжностей для проведення.") })); return; }
    setInvImportBusy(true);
    try {
      const r: any = await api.post("/api/stock-documents/import-inventory/", { commit: true, items, comment: `Інвентаризація (імпорт) ${pFrom}…${pTo}`, doc_date: pTo, warehouse: whs[0]?.id });
      if (r?.committed) {
        setInvImportOpen(false);
        loadProducts(); loadInvHistory(1); setInvScreen("history"); setInvHistSel(null);
      ...
      const msg = e?.response?.data?.detail || e?.message || "";
      setInvImportRes((p: any) => ({ ...(p || {}), error: (t("Не удалось провести. ","Не вдалося провести. ") + msg) }));
    }
    setInvImportBusy(false);
  }
  const invImportChanged = invImportRows.filter((r) => r.fact !== "" && r.fact !== null && r.fact !== undefined && Number(r.fact) !== Number(r.book)).length;
  ...
  return (
    <div className="scroll pad fade">
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ width: "min(640px,94vw)", height: "100%", background: "#fff", overflow: "auto", padding: "18px 22px" }}>
            ...
            {cardEdit ? (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, margin: "12px 0" }}>
                ...
                <label style={{ fontSize: 12 }} className="muted" title={t("Сколько ЕДИНИЦ ТОВАРА нужно на 1 м² для ФИНИШНОГО результата (со всеми слоями). В сделке количество считается само: площадь × расход.\n\nВесовой товар (кг): расход = кг на 1 м². Пример: Галатея 0,15.\nШтучный (бутылка/ведро): расход = 1 ÷ на сколько м² хватает упаковки. Примеры: грунт 100 г хватает на 4 м² → 0,25; грунт 1 л на 40 м² → 0,025. Количество штук округляется вверх.\n\nПусто = не считать.","Скільки ОДИНИЦЬ ТОВАРУ треба на 1 м² для ФІНІШНОГО результату (з усіма шарами). У сделці кількість рахується сама: площа × витрата.\n\nВаговий товар (кг): витрата = кг на 1 м². Приклад: Галатея 0,15.\nШтучний (пляшка/відро): витрата = 1 ÷ на скільки м² вистачає упаковки. Приклади: ґрунт 100 г вистачає на 4 м² → 0,25; ґрунт 1 л на 40 м² → 0,025. Кількість штук округлюється вгору.\n\nПорожньо = не рахувати.")}>{t("Расход на 1 м²","Витрата на 1 м²")}<input type="number" step="0.0001" value={cardEdit.consumption_per_m2} onChange={(e) => setCardEdit({ ...cardEdit, consumption_per_m2: e.target.value })} placeholder={t("напр. 0.15","напр. 0.15")} style={{ width: "100%", height: 34, border: "1px solid #cbd5e1", borderRadius: 7, padding: "0 8px", marginTop: 2 }} /></label>
                <label style={{ fontSize: 12 }} className="muted" title={t("Сколько килограммов в 1 литре (из техлиста, напр. 1,5). Нужна, чтобы посчитать объём и подобрать тару: литры = вес ÷ плотность.","Скільки кілограмів у 1 літрі (з техлиста, напр. 1,5). Потрібна, щоб порахувати обʼєм і підібрати тару: літри = вага ÷ щільність.")}>{t("Плотность, кг/л","Щільність, кг/л")}<input type="number" step="0.001" value={cardEdit.density_kg_l} onChange={(e) => setCardEdit({ ...cardEdit, density_kg_l: e.target.value })} placeholder={t("напр. 1.5","напр. 1.5")} style={{ width: "100%", height: 34, border: "1px solid #cbd5e1", borderRadius: 7, padding: "0 8px", marginTop: 2 }} /></label>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
              {[[t("Розничная цена","Роздрібна ціна"), Number(card.price).toLocaleString("ru") + " " + (card.currency || "грн")],
                ...
                ...(Number((card as any).consumption_per_m2) > 0 ? [[t("Расход на 1 м²","Витрата на 1 м²"), Number((card as any).consumption_per_m2) + " " + (card.unit || "")]] : []),
                ...(Number((card as any).density_kg_l) > 0 ? [[t("Плотность","Щільність"), Number((card as any).density_kg_l) + " кг/л"]] : []),
            ...
            {(canTabInv || canEdit) && card && (
              <div style={{ margin: "0 0 16px" }}>
                <RepackForm defaultTarget={card} compact onDone={() => { refreshCard(); loadProducts(); api.get<Movement[]>(`/api/products/${card.id}/movements/`).then(setMovements).catch(() => {}); }} />
              </div>
            )}
      ...
      {view === "inv" && (
        <div style={{ background: "#fff", borderRadius: 8, border: "1px solid #e2e8f0", padding: 22, maxHeight: "calc(100vh - 170px)", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 10, flexWrap: "wrap" }}>
              ...
              <span className="muted" style={{ fontSize: 12 }}>{t("Начальный (факт переучёта) + Поступление − Продано ± розлив = Конечный расчётный. Плюс контрольные пересчёты = В системе. Расхождение = Факт − В системе.","Початковий (факт переобліку) + Надходження − Продано ± розлив = Кінцевий розрахунок. Плюс контрольні переобліки = В системі. Розбіжність = Факт − В системі.")}</span>
            </div>
                  ...
                  {sheet.map((r) => {
                    ...
                    return (
                      <tr key={r.id}>
                        ...
                        <td style={{ fontWeight: 600 }} title={t("Сколько система числит сейчас. Именно от этой цифры считается расхождение при проведении инвентаризации","Скільки система числить зараз. Саме від цієї цифри рахується розбіжність при проведенні інвентаризації")}>{r.book.toLocaleString("ru")}</td>
              ...
              <div style={{ overflowY: "auto", flex: 1 }}>
                {!invHistSel && (<>
                  ...
                  {invHistDocs.map((d) => {
                    ...
                    return (
                      <div key={d.id} onClick={async () => { setInvHistSel(d); try { const det: any = await api.get(`/api/stock-documents/${d.id}/inv-detail/`); setInvHistSel((cur: any) => (cur && cur.id === d.id ? { ...cur, items: det.items } : cur)); } catch { /* показываем как есть */ } }} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 8px", borderBottom: "1px solid #f1f5f9", cursor: "pointer", opacity: cancelled ? 0.6 : 1 }}>
                    ...
                    <table style={{ width: "100%", fontSize: 13 }}>
                      ...
                      <tbody>
                        {(invHistSel.items || []).map((it: any) => {
                          ...
                          const bf = it.before != null ? Number(it.before) : null; const fc = it.fact != null ? Number(it.fact) : null;
                          return (
                            <tr key={it.id}>
                              <td>{it.product_name}</td>
                              <td className="muted" style={{ textAlign: "right" }}>{bf != null ? bf.toLocaleString("ru") : "—"}</td>
                              <td style={{ textAlign: "right", fontWeight: 600 }}>{fc != null ? fc.toLocaleString("ru") : "—"}</td>
                              <td style={{ textAlign: "right", color: q < 0 ? "#dc2626" : q > 0 ? "#16a34a" : "#94a3b8", fontWeight: 600 }}>{q > 0 ? "+" : ""}{q}</td>
                              {showCost && <><td className="muted" style={{ textAlign: "right" }}>{pr ? pr.toLocaleString("ru") : "—"}</td>
                              <td style={{ textAlign: "right" }}>{pr ? (Math.round(q * pr * 100) / 100).toLocaleString("ru") + " ₴" : "—"}</td></>}
                            </tr>
                          );
                        })}
      ...
      {/* Импорт инвентаризации: фото (ИИ) / файл / вставка → редактируемое превью */}
      {invImportOpen && (
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: 12, padding: 24, width: 820, maxWidth: "95vw", maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", marginBottom: 8 }}>
              <label className="btn btn-light" style={{ cursor: "pointer" }}>
                <Icon n="📷" size={14} /> {t("Добавить фото листов","Додати фото листів")}
                <input type="file" accept="image/*" multiple capture="environment" style={{ display: "none" }} onChange={(e) => addInvImportPhotos(e.target.files)} />
              </label>
              {invImportImages.length > 0 && <span className="muted" style={{ fontSize: 12 }}>{t("фото","фото")}: <b>{invImportImages.length}</b> / 20 {t("за раз (можно грузить пачками)","за раз (можна вантажити пачками)")}</span>}
            </div>
            {invImportImages.length > 0 && (
              <div style={{ display: "flex", gap: 8, overflowX: "auto", padding: "2px 0 8px", marginBottom: 4 }}>
                {invImportImages.map((src, i) => (
                  <span key={i} style={{ position: "relative", flex: "0 0 auto" }}>
                    <img src={src} alt="" style={{ height: 52, borderRadius: 6, border: "1px solid #e2e8f0" }} />
                    <button onClick={() => setInvImportImages((p) => p.filter((_, j) => j !== i))} title={t("Убрать","Прибрати")}
                      style={{ position: "absolute", top: -6, right: -6, width: 18, height: 18, borderRadius: 9, border: "none", background: "#dc2626", color: "#fff", fontSize: 11, cursor: "pointer", lineHeight: "18px", padding: 0 }}>×</button>
                  </span>
                ))}
              </div>
            )}

            {/* Источник: файл / вставка (если фото не выбраны) */}
            {invImportImages.length === 0 && (
              <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", marginBottom: 8 }}>
                <span className="muted" style={{ fontSize: 12 }}>{t("или","або")}</span>
                <label className="btn btn-light" style={{ cursor: "pointer" }}>
                  <Icon n="📎" size={14} /> {t("Файл Excel/CSV","Файл Excel/CSV")}
                ...
                {!invImportXlsx && !invImportFileName && (
                  <input value={invImportText} onChange={(e) => { setInvImportText(e.target.value); setInvImportRes(null); }} placeholder={t("или вставьте: Артикул;Факт","або вставте: Артикул;Факт")}
                    style={{ flex: 1, minWidth: 200, height: 34, border: "1px solid #cbd5e1", borderRadius: 8, padding: "0 10px", fontSize: 13, fontFamily: "monospace" }} />
                )}
              </div>
            )}
            ...
            {invImportRows.length > 0 && (
              <div style={{ marginBottom: 10, display: "flex", flexDirection: "column", flex: 1, minHeight: 0, border: "1px solid #f1f5f9", borderRadius: 8 }}>
                <div style={{ background: "#f8fafc", padding: "8px 10px", fontSize: 13, borderBottom: "1px solid #e2e8f0", display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  <span>{t("Позиций","Позицій")}: <b>{invImportRows.length}</b> · {t("с расхождением","з розбіжністю")}: <b style={{ color: "#b45309" }}>{invImportChanged}</b>{invImportRes && (invImportRes.not_found || []).length > 0 ? <> · {t("не найдено","не знайдено")}: <b style={{ color: "#dc2626" }}>{invImportRes.not_found.length}</b></> : null}</span>
                  <span style={{ flex: 1 }} />
                  <span className="muted" style={{ fontSize: 12 }}>{t("строк","рядків")}:</span>
                  {PAGE_SIZES.map((sz) => (
                    <button key={sz} onClick={() => { setInvImportPageSize(sz); setInvImportPage(1); }} style={{ fontSize: 12, padding: "3px 9px", borderRadius: 6, cursor: "pointer", border: "1px solid " + (invImportPageSize === sz ? "var(--brand)" : "#cbd5e1"), background: invImportPageSize === sz ? "var(--brand)" : "#fff", color: invImportPageSize === sz ? "#fff" : "#475569" }}>{sz}</button>
                  ))}
                </div>
                <div style={{ overflowY: "auto", flex: 1, minHeight: 0 }}>
                  <table style={{ width: "100%", fontSize: 12 }}>
                    <thead><tr>{[t("Артикул","Артикул"), t("Товар","Товар"), t("Учёт","Облік"), t("Факт","Факт"), t("Разница","Різниця"), ""].map((h, i) => <th key={i} style={{ textAlign: "left", padding: "5px 8px", position: "sticky", top: 0, background: "#fff", boxShadow: "inset 0 -1px 0 #e2e8f0" }}>{h}</th>)}</tr></thead>
                    <tbody>
                      {invImportRows.slice((invImportPage - 1) * invImportPageSize, invImportPage * invImportPageSize).map((r, li) => {
                        const gi = (invImportPage - 1) * invImportPageSize + li;
                        const delta = (r.fact === "" || r.fact === null || r.fact === undefined) ? 0 : Number(r.fact) - Number(r.book);
                        return (
                          <tr key={r.id} style={{ background: delta !== 0 ? "#fffbeb" : "transparent" }}>
                            <td style={{ padding: "4px 8px" }} className="muted">{r.sku || "—"}</td>
                            <td style={{ padding: "4px 8px" }}>{r.name} <span className="muted">{r.unit}</span></td>
                            <td style={{ padding: "4px 8px" }}>{r.book}</td>
                            <td style={{ padding: "4px 8px" }}>
                              <input type="number" value={r.fact} onChange={(e) => setInvImportRows((rows) => rows.map((x, j) => j === gi ? { ...x, fact: e.target.value } : x))}
                                style={{ width: 76, height: 26, border: "1px solid #cbd5e1", borderRadius: 6, padding: "0 6px" }} />
                            </td>
                            <td style={{ padding: "4px 8px", fontWeight: 600, color: delta < 0 ? "#dc2626" : delta > 0 ? "#16a34a" : "#94a3b8" }}>{delta > 0 ? "+" : ""}{Math.round(delta * 100) / 100 || 0}</td>
                            <td style={{ padding: "4px 8px" }}><button onClick={() => setInvImportRows((rows) => rows.filter((_, j) => j !== gi))} title={t("Убрать","Прибрати")} style={{ border: "none", background: "transparent", color: "#94a3b8", cursor: "pointer", fontSize: 15 }}>×</button></td>
                          </tr>
                        );
                      })}
                    </tbody>
                ...
                {invImportRows.length > invImportPageSize && (
                  <div style={{ display: "flex", gap: 6, alignItems: "center", justifyContent: "center", padding: "6px 10px", borderTop: "1px solid #f1f5f9" }}>
                    <button className="btn btn-light" disabled={invImportPage <= 1} onClick={() => setInvImportPage((p) => Math.max(1, p - 1))}>{t("← Назад","← Назад")}</button>
                    <span style={{ fontSize: 12 }}>{t("Стр.","Стор.")} <b>{invImportPage}</b> {t("из","з")} <b>{Math.max(1, Math.ceil(invImportRows.length / invImportPageSize))}</b></span>
                    <button className="btn btn-light" disabled={invImportPage >= Math.ceil(invImportRows.length / invImportPageSize)} onClick={() => setInvImportPage((p) => Math.min(Math.ceil(invImportRows.length / invImportPageSize), p + 1))}>{t("Вперёд →","Вперед →")}</button>
                  </div>
                )}
                {invImportRes && (invImportRes.not_found || []).length > 0 && (
...
// 19.09.2026: постачальник товару для заявок на дозамовлення (контакти з типом «Постачальник»)
function SupplierPick({ value, onChange }: { value: any; onChange: (v: any) => void }) {
  const [opts, setOpts] = useState<any[]>([]);
  useEffect(() => { api.get<any>("/api/contacts/?kind=supplier&page_size=300").then((r: any) => setOpts(r.results || r || [])).catch(() => setOpts([])); }, []);
  return (
    <select value={value || ""} onChange={(e) => onChange(e.target.value ? Number(e.target.value) : null)} style={{ width: "100%", height: 34, border: "1px solid #cbd5e1", borderRadius: 7, padding: "0 6px", marginTop: 2 }}>
      <option value="">— з останнього приходу —</option>
      {opts.map((c: any) => <option key={c.id} value={c.id}>{c.name || [c.first_name, c.last_name].filter(Boolean).join(" ") || c.company || ("#" + c.id)}</option>)}
    </select>
  );
}
