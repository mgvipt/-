                  ...
                  {sheet.map((r) => {
                    ...
                    return (
                      <tr key={r.id}>
                        ...
                        <td style={{ background: "#eff5ff" }} title={t("Контрольные пересчёты ВНУТРИ периода: дата → сколько насчитали, и сколько система дописала","Контрольні переобліки ВСЕРЕДИНІ періоду: дата → скільки нарахували, і скільки система дописала")}>{(r.controls && r.controls.length) ? r.controls.map((c) => (<div key={c.date} style={{ whiteSpace: "nowrap", fontSize: 11 }}>{c.date.split("-").reverse().slice(0,2).join(".")} → <b>{c.fact.toLocaleString("ru")}</b> <span style={{ color: c.delta > 0 ? "#16a34a" : "#dc2626" }}>({c.delta > 0 ? "+" : ""}{c.delta.toLocaleString("ru")})</span></div>)) : <span className="muted">—</span>}</td>
