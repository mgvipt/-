          ...
          <div onClick={(e) => e.stopPropagation()} style={{ width: "min(640px,94vw)", height: "100%", background: "#fff", overflow: "auto", padding: "18px 22px" }}>
            ...
            {cardEdit ? (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, margin: "12px 0" }}>
                ...
                <label style={{ fontSize: 12 }} className="muted" title={t("Сколько единиц товара нужно на 1 м² для ФИНИШНОГО результата (со всеми слоями). Если заполнено — в сделке количество считается само: площадь × расход. Пусто = не считать.","Скільки одиниць товару треба на 1 м² для ФІНІШНОГО результату (з усіма шарами). Якщо заповнено — у сделці кількість рахується сама: площа × витрата. Порожньо = не рахувати.")}>{t("Расход на 1 м²","Витрата на 1 м²")}<input type="number" step="0.0001" value={cardEdit.consumption_per_m2} onChange={(e) => setCardEdit({ ...cardEdit, consumption_per_m2: e.target.value })} placeholder={t("напр. 0.15","напр. 0.15")} style={{ width: "100%", height: 34, border: "1px solid #cbd5e1", borderRadius: 7, padding: "0 8px", marginTop: 2 }} /></label>
