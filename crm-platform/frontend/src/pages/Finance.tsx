...
function Directions() {
  ...
  const [to, setTo] = useState(today());
  const [d, setD] = useState<any>(null);
  const [dview, setDview] = useState("dirs");
  const { t } = useLang();
  const isMobile = useIsMobile();
  ...
  return (
    <div className="panel" style={{ margin: 0 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        {dview === "dirs" && <button className="btn btn-primary" style={{ fontSize: 13 }} title={t("Добавить новое направление (проект). Появится в журнале, планировании и аналитике.","Додати новий напрямок (проект). Зʼявиться у журналі, плануванні й аналітиці.")} onClick={() => setEdit({ name: "", plan_income: 0, plan_expense: 0 })}>+ {t("Направление","Напрямок")}</button>}
      </div>
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", margin: "8px 0 2px" }}>
        {([["dirs", t("Направления","Напрямки")], ["category", t("По категориям","По категоріях")], ["counterparty", t("По контрагентам","По контрагентах")], ["month", t("По месяцам","По місяцях")], ["matrix", t("Матрица","Матриця")]] as [string,string][]).map(([k, lbl]) => <button key={k} className={"btn" + (dview === k ? " btn-primary" : " btn-light")} style={{ fontSize: 12.5 }} onClick={() => setDview(k)}>{lbl}</button>)}
      </div>
      <Period from={from} to={to} set={(f, tt) => { setFrom(f); setTo(tt); }} />
      {dview !== "dirs" ? <BreakdownView by={dview} from={from} to={to} /> : !d ? <div className="spin">{t("Загрузка…","Завантаження…")}</div> : (<>
...
function BreakdownView({ by, from, to }: { by: string; from: string; to: string }) {
  const { t } = useLang();
  const [d, setD] = useState<any>(null);
  ...
  if (!d) return <div className="spin" style={{ marginTop: 10 }}>{t("Загрузка…", "Завантаження…")}</div>;
  return (
...
function ManagerDealsModal({ user, period, onClose }: { user: any; period: string; onClose: () => void }) {
  ...
  /* ВИСУВНА ПАНЕЛЬ СПРАВА: не перекриває екран затемненням — можна далі працювати
     в основному вікні; кнопка «−» згортає в смужку, «✕» закриває. */
  if (min0) return (
    <div onClick={() => setMin0(false)} title={t("Развернуть", "Розгорнути")}
      style={{ position: "fixed", right: 0, top: "35%", zIndex: 1000, background: "#2563eb", color: "#fff", padding: "10px 8px", borderRadius: "10px 0 0 10px", cursor: "pointer", fontSize: 12, fontWeight: 700, writingMode: "vertical-rl", boxShadow: "-3px 0 12px rgba(15,23,42,.25)" }}>
      💰 {user.name} · {money(d?.total || 0)}
    </div>
  );
  ...
  return (
    <div style={{ position: "fixed", right: 0, top: 0, bottom: 0, width: w, zIndex: 1000, background: "#fff", boxShadow: "-8px 0 28px rgba(15,23,42,.22)", display: "flex", flexDirection: "column", borderLeft: "1px solid #e2e8f0" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", borderBottom: "1px solid #e5eaf1", background: "#f8fafc", flexShrink: 0 }}>
        ...
        <button className="btn btn-light" style={{ height: 28, padding: "0 10px" }} title={t("Свернуть в полоску — можно работать в основном окне", "Згорнути в смужку — можна працювати в основному вікні")} onClick={() => setMin0(true)}>—</button>
        <button className="btn btn-light" style={{ height: 28, padding: "0 10px" }} onClick={onClose}>✕</button>
      </div>

      {!d ? <div className="spin" style={{ padding: 24 }}>{t("Загрузка…", "Завантаження…")}</div> : (
        <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
          {/* ── ПІДСУМКИ + ДНІ (не скролиться разом з таблицею) ── */}
          <div style={{ padding: "9px 14px 0", flexShrink: 0 }}>
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap", fontSize: 12.5, marginBottom: 8 }}>
              <span>{t("Поступило", "Надійшло")}: <b style={{ color: "#16a34a", fontSize: 14 }}>{money(d.total)}</b></span>
              {d.total_fee ? <span>{t("комиссия", "комісія")}: <b style={{ color: "#dc2626" }}>−{money(d.total_fee)}</b></span> : null}
              {d.total_net != null ? <span>{t("чисто", "чисто")}: <b style={{ color: "#047857", fontSize: 14 }}>{money(d.total_net)}</b></span> : null}
              <span className="muted">{d.deals} {t("сделок", "угод")} · {d.count} {t("операций", "операцій")}</span>
            </div>
            {(d.by_day || []).length > 0 && (
              <div style={{ marginBottom: 8 }}>
            ...
            {(d.by_funnel || []).length > 1 && (
              <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginBottom: 8 }}>
                {(d.by_funnel || []).map((b: any) => (
                  <div key={b.funnel} style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 7, padding: "3px 8px", fontSize: 11.5 }}>
                    <span style={{ color: "#1e40af", fontWeight: 600 }}>{b.funnel}</span>: <b style={{ color: "#16a34a" }}>{money(b.amount)}</b> <span className="muted">· {b.deals}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            <table style={{ width: "100%", fontSize: 12.5, minWidth: 900, borderCollapse: "separate", borderSpacing: 0 }}>
              <thead>
                <tr style={{ fontSize: 10.5, textAlign: "left", color: "#475569" }}>
                  {[[t("Сделка", "Угода"), ""], [t("Воронка", "Воронка"), ""], [t("Клиент", "Клієнт"), ""],
                    [t("Сумма сделки", "Сума угоди"), "r"], [t("Поступило", "Надійшло"), "r"],
                    [t("Комиссия", "Комісія"), "r"], [t("Чисто", "Чисто"), "r"], [t("Дата оплаты", "Дата оплати"), "r"]].map(([lb, al]: any, i: number) => (
                    <th key={i} style={{ ...cell, textAlign: al === "r" ? "right" : "left", position: "sticky", top: 0, background: "#f1f5f9", zIndex: 2, borderBottom: "1px solid #e2e8f0", fontWeight: 700 }}>{lb}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {byDayList && (d.rows_by_day || []).length > 0 ? (d.rows_by_day || []).flatMap((g: any) => ([
                  ...
                  <tr key={row.deal_id} onClick={() => setOpenDeal(row.deal_id)} style={{ borderTop: "1px solid #eef2f7", cursor: "pointer" }} title={t("Открыть сделку", "Відкрити угоду")}>
                    ...
                    <td style={cell}><span style={{ fontSize: 10.5, background: "#eff6ff", color: "#1e40af", borderRadius: 6, padding: "1px 6px" }}>{row.funnel || "—"}</span></td>
                    <td style={{ ...cell, maxWidth: 160, overflow: "hidden", textOverflow: "ellipsis" }}>{row.client || "—"}</td>
                    <td style={{ ...cell, textAlign: "right", color: "#475569" }}>{row.deal_amount == null ? "—" : money(row.deal_amount)}{(row.remaining != null && row.remaining > 1) ? <div style={{ fontSize: 10, color: "#b45309" }}>{t("ещё", "ще")} {money(row.remaining)}</div> : null}</td>
                    <td style={{ ...cell, textAlign: "right", fontWeight: 700, color: "#16a34a" }}>{money(row.amount)}</td>
                    <td style={{ ...cell, textAlign: "right", color: row.fee ? "#dc2626" : "#94a3b8" }}>{row.fee ? "−" + money(row.fee) : "—"}</td>
                    <td style={{ ...cell, textAlign: "right", fontWeight: 700, color: "#047857" }}>{money(row.net != null ? row.net : row.amount)}</td>
                    <td style={{ ...cell, textAlign: "right", color: "#64748b" }}>{row.last_date ? new Date(row.last_date).toLocaleDateString("ru") : "—"}</td>
                  </tr>
                ))}
...
function Salary() {
  ...
  // 15.09.2026 (Олег): блок «Стара формула CRM (довідково)» прибрано — ЗП лише за ставками, вкладка швидша.
  // Кнопка «Статистика» менеджера лишилась: список людей — з «Планів» (легкий запит замість старого розрахунку).
  useEffect(() => { api.get<any>(`/api/manager-plans/?period=${period}&page_size=100`).then((d) => setMgrs(Array.isArray(d) ? d : (d.results || []))).catch(() => setMgrs([])); }, [period]);
  return (
    <>
      <div style={{ display: "flex", gap: 8, alignItems: "center", margin: "0 0 10px", flexWrap: "wrap" }}>
        <span className="muted" style={{ fontSize: 13 }}>{t("Месяц","Місяць")}:</span>
        <input type="month" value={period} onChange={(e) => setPeriod(e.target.value)} style={{ height: 32, border: "1px solid #cbd5e1", borderRadius: 6, padding: "0 8px" }} />
        {mgrs.length > 0 && <span className="muted" style={{ fontSize: 12, marginLeft: 8 }}>{t("Статистика менеджера","Статистика менеджера")}:</span>}
        {mgrs.map((m: any) => (
          <button key={m.id ?? m.user} className="btn btn-light" style={{ height: 28, padding: "0 10px", fontSize: 12, display: "inline-flex", alignItems: "center", gap: 5 }}
            onClick={() => setSelDeals({ id: m.user, name: m.user_name })}><Icon n="📊" size={13} /> {m.user_name}</button>
        ))}
      </div>
...
function SupplierMapModal({ docId, onClose, onDone }: { docId: number; onClose: () => void; onDone: () => void }) {
  ...
  const addLine = () => setLines((ls) => [...ls, { their_name: "", qty: 1, price: 0, sum: 0, factor: 1, product_id: null, product_name: "", retail: "" }]);
  const delLine = (i: number) => setLines((ls) => ls.filter((_, j) => j !== i));
  // подтянуть розничную цену из номенклатуры для строк, уже сопоставленных с товаром (из правил), где розница пуста
  useEffect(() => {
    if (!prods.length || !lines.length) return;
    setLines((ls) => {
      let changed = false;
      const next = ls.map((l) => {
        if (l.product_id && (l.retail === undefined || l.retail === "" || l.retail === null)) {
          const p = prods.find((x) => x.id === l.product_id);
          if (p && Number(p.price) > 0) { changed = true; return { ...l, retail: String(p.price) }; }
        }
        return l;
      });
  ...
  const post = async () => {
    ...
    try {
      ...
      // сохранить розничную цену в карточки товаров, где она вписана/изменена
      for (const l of lines) { const rp = Number(String(l.retail ?? "").replace(",", ".")); if (l.product_id && rp > 0) { await api.patch(`/api/products/${l.product_id}/`, { price: rp }).catch(() => {}); } }
      ...
      <div className="panel" style={{ maxWidth: 920, margin: "16px auto", background: "#fff" }} onClick={(e) => e.stopPropagation()}>
        ...
        <table style={{ width: "100%", fontSize: 13, borderCollapse: "collapse" }}>
          <thead><tr style={{ textAlign: "left", borderBottom: "1px solid #e2e8f0" }}>
            <th style={{ padding: 4 }}>{t("Его товар", "Його товар")}</th><th>{t("Наш товар (склад)", "Наш товар (склад)")}</th><th style={{ width: 58 }} title={t("Кол-во в единицах поставщика (напр. вёдра)", "К-сть в одиницях постачальника (напр. відра)")}>{t("К-во", "К-сть")}</th><th style={{ width: 66 }} title={t("Сколько наших единиц (кг) в 1 единице поставщика (ведро). 1 = одинаковые", "Скільки наших одиниць (кг) в 1 одиниці постачальника (відро). 1 = однакові")}>{t("Коэф →скл", "Коеф →скл")}</th><th style={{ width: 92 }} title={t("Закупочная цена за единицу склада — можно исправить, сумма пересчитается", "Закупівельна ціна за одиницю складу — можна виправити, сума перерахується")}>{t("Закупка", "Закупівля")}</th><th style={{ width: 98 }} title={t("Итоговая сумма строки со скидкой", "Підсумкова сума рядка зі знижкою")}>{t("Сумма", "Сума")}</th><th style={{ width: 86 }} title={t("Розничная цена за единицу — из номенклатуры (если есть), можно изменить; сохранится в карточку", "Роздрібна ціна за одиницю — з номенклатури (якщо є), можна змінити; збережеться в картку")}>{t("Розница", "Роздрібна")}</th><th style={{ width: 30 }}></th></tr></thead>
          <tbody>
            {!lines.length && (
              <tr><td colSpan={7} className="muted" style={{ padding: "10px 4px", fontSize: 12.5 }}>
            ...
            {lines.map((l, i) => (
              <tr key={i} style={{ borderBottom: "1px solid #f1f5f9" }}>
                ...
                <td><input type="number" className="num-tight" value={l._uc != null ? l._uc : (() => { const q = Number(l.qty) || 0; const f = Number(l.factor) || 1; const tot = (l.sum != null && l.sum !== "") ? Number(l.sum) : q * (Number(l.price) || 0); const sq = q * f; return sq ? String(Math.round(tot / sq * 100) / 100) : ""; })()} onChange={(e) => { const raw = e.target.value; const v = Number(String(raw).replace(",", ".")) || 0; const q = Number(l.qty) || 0; const f = Number(l.factor) || 1; setLine(i, { _uc: raw, sum: Math.round(v * q * f * 100) / 100 }); }} onBlur={() => setLine(i, { _uc: undefined })} title={t("Закупочная цена за единицу — можно исправить; сумма строки пересчитается", "Закупівельна ціна за одиницю — можна виправити; сума рядка перерахується")} placeholder={t("закуп", "закуп")} style={{ width: 88, height: 30, border: "1px solid #cbd5e1", borderRadius: 6, padding: "0 6px", fontSize: 12, textAlign: "right" }} /></td><td><input type="number" className="num-tight" value={l.sum ?? ""} placeholder={String(Math.round((Number(l.qty) || 0) * (Number(l.price) || 0)))} onChange={(e) => setLine(i, { sum: e.target.value === "" ? null : parseFloat(e.target.value) })} style={{ width: 96, height: 30, border: "1px solid #cbd5e1", borderRadius: 6, padding: "0 6px", fontSize: 12, textAlign: "right" }} /></td>
                <td><input type="number" className="num-tight" value={l.retail ?? ""} onChange={(e) => setLine(i, { retail: e.target.value })} placeholder={t("розн.", "розн.")} title={t("Розничная цена за единицу — сохранится в карточку товара", "Роздрібна ціна за одиницю — збережеться в картку товару")} style={{ width: 84, height: 30, border: "1px solid #a7f3d0", borderRadius: 6, padding: "0 6px", fontSize: 12, textAlign: "right" }} /></td>
