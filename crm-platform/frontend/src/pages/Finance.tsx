      ...
      <div className="panel" style={{ maxWidth: 920, margin: "16px auto", background: "#fff" }} onClick={(e) => e.stopPropagation()}>
        ...
        <table style={{ width: "100%", fontSize: 13, borderCollapse: "collapse" }}>
          <thead><tr style={{ textAlign: "left", borderBottom: "1px solid #e2e8f0" }}>
            <th style={{ padding: 4 }}>{t("Его товар", "Його товар")}</th><th>{t("Наш товар (склад)", "Наш товар (склад)")}</th><th style={{ width: 58 }} title={t("Кол-во в единицах поставщика (напр. вёдра)", "К-сть в одиницях постачальника (напр. відра)")}>{t("К-во", "К-сть")}</th><th style={{ width: 66 }} title={t("Сколько наших единиц (кг) в 1 единице поставщика (ведро). 1 = одинаковые", "Скільки наших одиниць (кг) в 1 одиниці постачальника (відро). 1 = однакові")}>{t("Коэф →скл", "Коеф →скл")}</th><th style={{ width: 98 }} title={t("Итоговая сумма строки со скидкой", "Підсумкова сума рядка зі знижкою")}>{t("Сумма", "Сума")}</th><th style={{ width: 86 }} title={t("Розничная цена за единицу — из номенклатуры (если есть), можно изменить; сохранится в карточку", "Роздрібна ціна за одиницю — з номенклатури (якщо є), можна змінити; збережеться в картку")}>{t("Розница", "Роздрібна")}</th><th style={{ width: 30 }}></th></tr></thead>
          <tbody>
            {!lines.length && (
