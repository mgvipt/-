        ...
        {tab === "creatives" && <>
          ...
          <div className="note" style={{ marginBottom: 12 }}>
            ...
            <div style={{ marginTop: 6, lineHeight: 1.55 }}>
              {t("Как читать: «Диалоги» — сколько людей написали (считает Meta). «Лиды CRM» — сколько из них удалось привязать к этому объявлению. Люди не теряются: в CRM попадают почти все, теряется МЕТКА объявления.", "Як читати: «Діалоги» — скільки людей написали (рахує Meta). «Ліди CRM» — скільки з них вдалося привʼязати до цього оголошення. Люди не губляться: у CRM потрапляють майже всі, губиться МІТКА.")}<br />
              ...
              • <b>{t("Наш «лид» = «Диалоги Meta»", "Наш «лід» = «Діалоги Meta»")}</b>: {t("у нас каждый написавший становится лидом в CRM. «Лиды Meta» — это её лид-формы (анкеты), у нас их нет, поэтому там 0.", "у нас кожен, хто написав, стає лідом у CRM. «Ліди Meta» — це її лід-форми (анкети), у нас їх немає, тому там 0.")}
            </div>
          </div>
  ...
  return <div className="panel" style={{ padding: 13 }}>
    ...
    <div style={metricGrid}>
      ...
      <Metric label={t("Диалоги Meta", "Діалоги Meta")} value={count(row.messages_started)}
        tip={t("Сколько человек начали с нами переписку в Direct после этого объявления. Считает сама Meta за 7 дней после клика. Почти все они попадают в CRM как лиды — но метку объявления получают не все, поэтому «Лиды CRM» ниже. Разрыв здесь — про потерю МЕТКИ, а не про потерю людей.", "Скільки людей почали з нами листування в Direct після цього оголошення. Рахує Meta за 7 днів після кліку. Майже всі вони потрапляють у CRM як ліди — але мітку оголошення отримують не всі, тому «Ліди CRM» нижчі. Розрив тут — про втрату МІТКИ, а не людей.")} />
      <Metric label={t("Лиды Meta", "Ліди Meta")} value={count(row.meta_leads)}
        tip={t("Для Meta «лид» — это ТОЛЬКО заполненная лид-форма (анкета прямо в Facebook/Instagram). У нас кампании настроены на ПЕРЕПИСКУ, лид-форм в них нет — поэтому здесь почти всегда 0. Это нормально, не ошибка. Твой «лид» (человек написал) у Meta называется «Диалоги» — смотри соседнюю плитку.", "Для Meta «лід» — це ТІЛЬКИ заповнена лід-форма (анкета прямо у Facebook/Instagram). У нас кампанії на ЛИСТУВАННЯ, лід-форм немає — тому тут майже завжди 0. Це нормально. Твій «лід» (людина написала) у Meta зветься «Діалоги».")} />
      <Metric label={t("Лиды CRM", "Ліди CRM")} value={count(row.crm_leads)}
        tip={t("Сколько лидов в нашей базе удалось ПРИВЯЗАТЬ именно к этому объявлению. ВАЖНО: это НЕ значит, что остальные не стали лидами — в CRM попадают почти все, кто написал, но метка объявления долетает не всегда (её передаёт только прямой канал Meta, а через ChatPlace метки нет). Поэтому цифра занижена: реальный результат объявления выше. Используй её для СРАВНЕНИЯ креативов между собой, а не как полное число клиентов.", "Скільки лідів у нашій базі вдалося ПРИВʼЯЗАТИ саме до цього оголошення. ВАЖЛИВО: це НЕ означає, що інші не стали лідами — у CRM потрапляють майже всі, хто написав, але мітка оголошення долітає не завжди (її передає лише прямий канал Meta, через ChatPlace мітки немає). Цифра занижена: реальний результат вищий. Використовуй для ПОРІВНЯННЯ креативів між собою.")} />
