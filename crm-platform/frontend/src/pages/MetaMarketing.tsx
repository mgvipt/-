        ...
        {tab === "creatives" && <>
          ...
          <div className="note" style={{ marginBottom: 12 }}>
            ...
            <div style={{ marginTop: 6, lineHeight: 1.55 }}>
              {t("Читай карточку как цепочку: Расход → Диалоги → Лиды CRM. Где обрыв — там проблема:", "Читай картку як ланцюжок: Витрати → Діалоги → Ліди CRM. Де обрив — там проблема:")}<br />
              • {t("много диалогов, мало лидов CRM — пишут не те люди (аудитория/оффер) или не успеваем отвечать", "багато діалогів, мало лідів CRM — пишуть не ті люди або не встигаємо відповідати")}<br />
              • {t("лиды есть, а продаж нет — менеджеры не дожимают", "ліди є, а продажів немає — менеджери не дотискають")}<br />
              • {t("расход идёт, а лидов CRM ноль — объявление стоит проверить или выключить", "витрати йдуть, а лідів CRM нуль — оголошення варто перевірити або вимкнути")}
            </div>
          </div>
  ...
  return <div className="panel" style={{ padding: 13 }}>
    ...
    <div style={metricGrid}>
      ...
      <Metric label={t("Диалоги Meta", "Діалоги Meta")} value={count(row.messages_started)}
        tip={t("Сколько человек начали с нами переписку в Direct после этого объявления. Считает сама Meta за 7 дней после клика. ВАЖНО: это ещё НЕ клиенты — многие спросят цену и пропадут. Сравнивай с «Лиды CRM»: большой разрыв значит, что пишут не те люди или мы не отвечаем вовремя.", "Скільки людей почали з нами листування в Direct після цього оголошення. Рахує Meta за 7 днів після кліку. Це ще НЕ клієнти. Порівнюй з «Ліди CRM»: великий розрив = пишуть не ті або ми не відповідаємо вчасно.")} />
      ...
      <Metric label={t("Лиды CRM", "Ліди CRM")} value={count(row.crm_leads)}
        tip={t("Сколько РЕАЛЬНЫХ карточек лидов появилось в нашей базе с меткой этого объявления. Каждого можно открыть и посмотреть поимённо. Считаются ВСЕ карточки с меткой — на любой стадии («Лид получен», «Взято в работу», «Контакт установлен» и т.д.), даже те, кого уже потеряли. Метка ставится один раз при первом обращении и остаётся с человеком навсегда. Это наша правда — по ней и судим о рекламе.", "Скільки РЕАЛЬНИХ карток лідів з'явилося в нашій базі з міткою цього оголошення. Кожного можна відкрити поіменно. Рахуються ВСІ картки з міткою — на будь-якій стадії, навіть втрачені. Мітка ставиться один раз і лишається назавжди.")} />
