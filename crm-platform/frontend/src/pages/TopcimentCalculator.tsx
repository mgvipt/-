...
import { api } from "../api";
type Row={key:string;name:string;unit:string;rate:number|null;quantity:number|null;pack:number|null;packs:number|null;purchase_quantity:number|null;pack_price:number|null;subtotal:number|null;needs_review:boolean;products:{id:number;name:string;updated_at:string;source_revisions:string[];needs_review:boolean}[]};
type Result={system:string;area:number;reserve:number;basis:string;rows:Row[];known_total:number;complete:boolean;technical_unverified:{key:string;name:string;products:Row["products"]}[];missing:string[];notes:string[];reference_date:string;eur_uah:string;systems:{id:string;name:string}[]};
...
export function TopcimentCalculator({onBack,onClose}:{onBack:()=>void;onClose:()=>void}) {
 ...
 {p.needs_review&&<span style={{color:"#92400e"}}>Потрібно погодити застосування та норму. </span>}
 ...
 <label style={{gridColumn:narrow?"1 / -1":undefined,minWidth:0}}>Ціни<br/><select style={{width:"100%",minHeight:40}} value={basis} onChange={e=>setBasis(e.target.value)}><option value="sale">Продажні ціни Wallcov</option><option value="reference">Архівний орієнтир: ціни / курс НБУ</option></select></label></div>
 ...
 {result.basis==="reference"&&<p style={{background:"#fff7ed",padding:12}}>Архівний довідковий розрахунок станом на {result.reference_date}: ціни та курс 1 € = {result.eur_uah} грн зафіксовані на цю дату й не оновлюються автоматично. Доставка, імпортні платежі та місцеві витрати не враховані. Продажну ціну Wallcov потрібно погодити.</p>}
