...
type Result={system:string;area:number;reserve:number;basis:string;rows:Row[];known_total:number;complete:boolean;technical_unverified:{key:string;name:string;products:Row["products"]}[];missing:string[];notes:string[];reference_date:string|null;eur_uah:string|null;reference_metadata:{status:"unknown"|"incomplete"|"consistent"|"mixed";sources:ReferenceSource[]};systems:{id:string;name:string}[]};
...
export function TopcimentCalculator({onBack,onClose}:{onBack:()=>void;onClose:()=>void}) {
 ...
 {result?.basis==="reference"&&r.reference_prices?.map(source=><small key={source.product_id} style={{display:"block",color:"#64748b"}}>{source.name}: ціна перевірена {source.date||"— дату уточнити"}; курс {source.eur_uah?`1 € = ${source.eur_uah} грн`:"уточнити"}.</small>)}
