...
export function MediaLibraryPicker({ conversationId, onSent, onClose }: { conversationId: number; onSent: (m: ChatMessage) => void; onClose: () => void }) {
  ...
  const colors = useMemo(() => Array.from(new Set(materialItems.map((x) => x.color_code).filter(Boolean))).filter((x) => x.toLowerCase().includes(query.toLowerCase())), [materialItems, query]);
  ...
  return <div style={{ position: "absolute", zIndex: 50, left: 0, bottom: 46, width: 390, maxWidth: "calc(100vw - 24px)", maxHeight: 470, overflow: "auto", background: "#fff", border: "1px solid #cbd5e1", borderRadius: 12, padding: 10, boxShadow: "0 12px 32px rgba(15,23,42,.2)" }}>
    ...
    {tab === "colors" && <>
      ...
      {screen === "materials" && <><div className="muted" style={{ fontSize: 12, marginBottom: 7 }}>Спочатку оберіть матеріал — тоді відкриються його каталог, кольори та інтер'єри.</div><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Пошук матеріалу" style={{ width: "100%", boxSizing: "border-box", padding: "7px 9px", border: "1px solid #cbd5e1", borderRadius: 7, fontSize: 12, marginBottom: 8 }} /><div style={{ display: "grid", gap: 7 }}>{materials.map((name) => { const group = colorItems.filter((x) => (x.material || "Матеріал без назви") === name); const codes = new Set(group.map((x) => x.color_code).filter(Boolean)).size; const preview = group.find((x) => x.url && x.kind !== "video"); return <button key={name} onClick={() => openMaterial(name)} className="btn" style={{ display: "flex", alignItems: "center", gap: 8, textAlign: "left" }}>{preview?.url && <img src={preview.url} style={{ width: 42, height: 42, objectFit: "cover", borderRadius: 6 }} />}<span><b>{name}</b><br /><span className="muted" style={{ fontSize: 11 }}>{codes} кольорів · {group.filter((x) => x.kind === "catalog").length} сторінок каталогу</span></span><span style={{ marginLeft: "auto" }}>›</span></button>; })}</div></>}
      {screen === "material" && <><b style={{ fontSize: 13 }}>{material}</b><div className="muted" style={{ fontSize: 12, margin: "3px 0 8px" }}>Сторінки каталогу та всі кольори матеріалу</div>{catalogPages.length > 0 && <div style={{ marginBottom: 10 }}><b style={{ fontSize: 12 }}>📖 Каталог</b><div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 7, marginTop: 5 }}>{catalogPages.map(card)}</div></div>}<input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Знайти код кольору" style={{ width: "100%", boxSizing: "border-box", padding: "7px 9px", border: "1px solid #cbd5e1", borderRadius: 7, fontSize: 12, marginBottom: 8 }} /><b style={{ fontSize: 12 }}>🎨 Кольори ({colors.length})</b><div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 7, marginTop: 5 }}>{colors.map((code) => { const sample = materialItems.find((x) => x.color_code === code && x.kind === "image"); const count = materialItems.filter((x) => x.color_code === code).length; return <button key={code} onClick={() => openColor(code)} style={{ border: "1px solid #dbe3ee", background: "#fff", padding: 5, borderRadius: 8, textAlign: "left", cursor: "pointer" }}>{sample?.url && <img src={sample.url} style={{ width: "100%", height: 65, objectFit: "cover", borderRadius: 5 }} />}<div style={{ fontSize: 11, marginTop: 3 }}><b>{code}</b><br /><span className="muted">{count} матеріал(ів) ›</span></div></button>; })}</div></>}
      {screen === "color" && <><b style={{ fontSize: 13 }}>{material} · {color}</b><div className="muted" style={{ fontSize: 12, margin: "3px 0 8px" }}>Еталон, відео та інтер'єри цього кольору</div><div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 7 }}>{colorAssets.map(card)}</div></>}
