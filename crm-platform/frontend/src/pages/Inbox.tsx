...
export default function Inbox() {
  ...
  const [libraryQuick, setLibraryQuick] = useState(false);  // 17.09.2026: «/» у порожньому полі — одразу швидкі відповіді (як у amoCRM)
  ...
  const [ropQ, setRopQ] = useState("");           // 22.09.2026: «Запитати ШІ-РОП про цей діалог»
          ...
          {(() => {
            ...
            const card = (c: Conversation) => (
              <div key={c.id} onClick={() => (selMode ? toggleSel(c.id) : openConv(c))}
                ...
                {DENS.av ? <Avatar src={(c as any).contact_avatar || ""} name={c.contact_name || c.title || "?"} cls={DENS.av} /> : null}
                ...
                {(() => {
                  ...
                  // ШІ-іконка (bot, як у всій CRM) — база; червоний лічильник — маленьким бейджем у куті
                  return (
              ...
              {msgs.map((m, i) => (
                ...
                <div id={`inbox-message-${m.id}`} className={"msg" + (m.direction === "out" ? " msg-out" : " msg-in")} data-internal={(m as any).internal ? "1" : ""}
                  ...
                  {m.attachments?.map((a: any, i: number) => (
                    ...
                    : (a.url && a.type === "button") ? <a key={i} href={a.url} target="_blank" rel="noreferrer" style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: 38, marginTop: 8, padding: "0 14px", borderRadius: 8, background: "#eef2ff", border: "1px solid #c7d2fe", color: "#1d4ed8", fontSize: 12.5, fontWeight: 700, textDecoration: "none" }}>{a.name || t("Открыть форму","Відкрити форму")}</a>
      ...
      <div style={{ background: "#fff", borderLeft: "1px solid #e2e8f0", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        {active && (
          <div style={{ padding: "10px 12px", borderBottom: "1px solid #e2e8f0", flexShrink: 0, background: "#f8fafc" }}>
            ...
            {ropQa && (
              <div style={{ marginTop: 8, background: ropQa.error ? "#FCEBEB" : "#E6F1FB", borderRadius: 8, padding: "8px 10px", maxHeight: 240, overflowY: "auto" }}>
