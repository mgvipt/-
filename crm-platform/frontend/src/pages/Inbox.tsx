          ...
          <>
            <div ref={headRef} style={{ minHeight: 52, background: "#fff", borderBottom: "1px solid #e2e8f0", display: "flex", alignItems: "center", flexWrap: "wrap", gap: 6, rowGap: 6, padding: "7px 14px" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                ...
                <button className="btn btn-green" style={{ height: 30, width: 30, padding: 0, flexShrink: 0, display: "inline-flex", alignItems: "center", justifyContent: "center" }} title={t("Позвонить клиенту","Подзвонити клієнту")}><Icon n="phone" size={15} /></button>
            ...
            <div style={{ background: "#fff", borderTop: "1px solid #e2e8f0", padding: "0 12px 12px" }}>
              ...
              <div style={{ position: "relative", flex: "0 0 auto" }}><button className="btn" type="button" style={{ background: "#f1f5f9", height: 38, width: 38, padding: 0, display: "inline-flex", alignItems: "center", justifyContent: "center" }} title={t("Библиотека: цвета, каталоги и быстрые ответы","Бібліотека: кольори, каталоги і швидкі відповіді")} onClick={() => setLibraryOpen((v) => !v)} disabled={sending}><Icon n="grid" size={17} /></button>{libraryOpen && active && <MediaLibraryPicker conversationId={active.id} onSent={(m) => setMsgs((ms) => [...ms, m])} onClose={() => setLibraryOpen(false)} onInsertText={(t) => setText((prev) => (prev ? prev + "\n" : "") + t)} clientName={active.contact_name || active.title || ""} />}</div>
