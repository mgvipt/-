...
export default function FunnelEditor({ funnel, onClose, onSaved }: { funnel: Funnel; onClose: () => void; onSaved: (f: Funnel) => void }) {
  ...
  function insertAt(i: number) { setRows((r) => [...r.slice(0, i), { name: "Нова стадія", color: PRESETS[r.length % PRESETS.length], is_won: false, is_lost: false }, ...r.slice(i)]); }
  ...
  async function save() {
    ...
    try {
      ...
      if (f.blocked && f.blocked.length) { setWarn(`Не видалено (є картки): ${f.blocked.join(", ")}`); setRows(f.stages.map((s) => ({ id: s.id, name: s.name, color: s.color, is_won: s.is_won, is_lost: s.is_lost }))); }
      else onSaved(f);
    } catch { setWarn("Помилка збереження"); }
    finally { setSaving(false); }
  }

  const PlusRow = ({ i }: { i: number }) => (
    <div style={{ height: 14, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <button onClick={() => insertAt(i)} title="Додати стадію сюди"
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 60 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: 16, padding: 22, width: 560, maxHeight: "86vh", overflowY: "auto", boxShadow: "0 24px 60px rgba(15,23,42,.35)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
          <h3 style={{ margin: 0, fontSize: 18 }}>Налаштування воронки</h3>
          <span className="muted" style={{ fontSize: 13 }}>· {funnel.name}</span>
        </div>
        <div className="muted" style={{ fontSize: 12, marginBottom: 12 }}>Перейменуй, зміни колір, додай «+» у будь-яке місце, перетягни ↑↓ або видали. «✓ Виграно» / «✕ Програно» позначають фінальні стадії.</div>
          ...
          <div key={i}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 10, padding: "8px 10px" }}>
              ...
              <input type="color" value={r.color} onChange={(e) => patch(i, { color: e.target.value })}
                title="Свій колір" style={{ width: 30, height: 30, border: "none", background: "none", padding: 0, cursor: "pointer" }} />
              <button onClick={() => patch(i, { is_won: !r.is_won, is_lost: false })} title="Стадія «Виграно»"
              ...
              <button onClick={() => patch(i, { is_lost: !r.is_lost, is_won: false })} title="Стадія «Програно»"
              ...
              <button onClick={() => remove(i)} title="Видалити стадію"
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
          <button className="btn btn-light" style={{ flex: 1 }} onClick={onClose}>Закрити</button>
          <button className="btn btn-primary" style={{ flex: 1 }} onClick={save} disabled={saving}>{saving ? "Збереження…" : "Зберегти воронку"}</button>
        </div>
      </div>
    </div>
  );
