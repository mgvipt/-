...
const TH: React.CSSProperties = { position: "sticky", top: 0, background: "#f1f5f9", padding: "5px 7px", fontWeight: 600, whiteSpace: "nowrap", textAlign: "left" };
const TD: React.CSSProperties = { padding: "4px 7px", whiteSpace: "nowrap" };
const moneyOrDash = (v: any) => (Number(v) ? money2(v) : <span className="muted">—</span>);

/** 15.09 (регламент v2): склад по угодах — колонки за кожним пунктом оплати; клік по угоді — «як пораховано» і записи. */
function DealGroups({ groups }: { groups: any[] }) {
