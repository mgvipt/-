  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 800, color: "#16a34a", fontSize: 13, letterSpacing: ".3px" }}>ВХІДНИЙ ДЗВІНОК</div>
          {ring.line && <div style={{ fontSize: 11, color: "#7c5cff" }}>📡 {ring.line}</div>}
        </div>
        <button onClick={close} title="Сховати" style={{ border: "none", background: "transparent", fontSize: 20, cursor: "pointer", color: "#94a3b8" }}>×</button>
      </div>
      <div style={{ fontSize: 19, fontWeight: 700, lineHeight: 1.2 }}>{ring.contact_name || "Невідомий номер"}</div>
      ...
      <button className="btn btn-green" style={{ width: "100%", height: 38 }}
        ...
        {ring.contact ? "Відкрити картку клієнта" : "Відкрити телефонію"}
      </button>
    </div>
  );
}
