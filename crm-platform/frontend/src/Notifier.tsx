...
export default function Notifier() {
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        const d = await api.get<{ last_in: number; latest: any; unread: number }>("/api/inbox/ping/");
  ...
  return (
    <div style={{ position: "fixed", top: 16, right: 16, zIndex: 99998, display: "flex", flexDirection: "column", gap: 10, width: 320, maxWidth: "90vw" }}>
      {toasts.map((to) => (
        <div key={to.id} onClick={() => { nav(`/inbox?c=${to.conv_id}`); setToasts((ts) => ts.filter((x) => x.id !== to.id)); }}
          style={{ background: "#fff", borderRadius: 14, boxShadow: "0 12px 34px rgba(20,40,80,.24)", border: "1px solid #e8edf3", borderLeft: "4px solid #C67D5F", padding: "11px 13px", cursor: "pointer" }}>
