...
(function () {
  ...
  var PHASES = {
    menstrual: { name: "Менструация", short: "Отдых", emoji: "🌑", color: "var(--p-menstrual)",
      today: "Энергии мало — и это нормально. Отдохни и будь в тепле.",
      feel: "Усталость, спазмы, нужен покой.", partner: "Забота: тепло, меньше нагрузки, эмпатия.",
    ...
    follicular: { name: "Фолликулярная", short: "Подъём", emoji: "🌱", color: "var(--p-follicular)",
      today: "Сил становится больше — хорошее время для нового.",
      feel: "Энергия и настроение растут.", partner: "Поддержи идеи, планируй активности.",
    ...
    ovulation: { name: "Овуляция", short: "Пик", emoji: "☀️", color: "var(--p-ovulation)",
      today: "Пик энергии и настроения. Ты сияешь.",
      feel: "Максимум сил и влечения.", partner: "Внимание и романтика. Контрацепция — вместе.",
      food: ["Лёгкая клетчатка", "Ягоды, антиоксиданты", "Больше воды", "Цинк: тыквенные семечки"],
      aff: ["Я уверена и сияю.", "Я достойна любви.", "Мой голос важен."], med: "gratitude" },
    luteal: { name: "Лютеиновая", short: "Спад", emoji: "🌘", color: "var(--p-luteal)",
      today: "Энергия снижается, возможен ПМС. Будь мягче к себе.",
      feel: "Спад, чувствительность, тяга к еде.", partner: "Терпение, помощь по быту, меньше споров.",
  ...
  var COURSE = [
    { id: "cycle", title: "Как устроен цикл", mins: 2, body: ["Цикл — плавная смена 4 фаз со своей энергией и настроением.", "🌑 отдых → 🌱 подъём → ☀️ пик → 🌘 спад. Это биология, а не «характер»."] },
    { id: "pms", title: "ПМС: как быть рядом", mins: 2, body: ["Перед месячными гормоны падают — отсюда перепады настроения и усталость.", "Помогает: терпение, помощь по быту, тёплое внимание. Избегай обесценивания."] },
    { id: "energy", title: "Энергия по фазам", mins: 2, body: ["Планы под фазу = забота. Пик сил — фолликулярная и овуляция.", "В менструацию и конец лютеиновой возьми быт на себя и не настаивай."] },
    { id: "intimacy", title: "Близость и согласие", mins: 2, body: ["Влечение меняется по циклу — «нормы» нет.", "Светофор близости показывает только сигнал. Спрашивай, а не предполагай."] },
    { id: "words", title: "Слова поддержки", mins: 2, body: ["Вместо «успокойся» — «я рядом, чем помочь?».", "Тепло, вода, обнять без повода. Спроси: поддержать или побыть в тишине?"] }
  ];
  ...
  var KB = [
    ...
    { k: ["либидо","влечен","желани"], a: "Либидо обычно выше около овуляции, ниже в ПМС — это нормально и индивидуально." },
  ...
  function screenPartner() {
    ...
    return head("Для партнёра") +
      ...
      '<div class="card signal-card"><div class="signal-badge">' + sig.em + '</div><div><h3 style="margin-bottom:2px">Сигнал близости: ' + sig.title + '</h3><p>' + sig.text + '</p></div></div>' +
      courseCard() +
